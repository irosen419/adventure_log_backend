...
animal_array = []
while counter < 13 do
    animal_url = "https://apiv3.iucnredlist.org/api/v3/species/page/#{counter}?token=#{IUCN_KEY}"
...
counter = 1
animal_array.each do |scientific_name|
    ...
    if url_response
        ...
        if result && result.length > 0
            if result[0]['language'] == 'eng' && result[0]['taxonname']
                ...
                Animal.create(scientific_name: scientific_name, common_name: result[0]['taxonname'])
                counter += 1
            end
        end
    end
